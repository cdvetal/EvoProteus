...
float __s =29.59207; //min:0 max:100
float __b =41.602623; //min:0 max:100
float __op =82.95632; //min:0 max:100
...
float __size=200.0; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 12);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(23,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
