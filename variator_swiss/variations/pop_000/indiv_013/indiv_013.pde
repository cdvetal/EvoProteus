...
float __h=104.65872; //min:0 max:360
float __s =41.18759; //min:0 max:100
float __b =44.014137; //min:0 max:100
float __op =84.04443; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=23; //min:10 max:30

float __pos=0.24482626; //min:0.1 max:0.4
float __size=134.25958; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 13);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(346,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
