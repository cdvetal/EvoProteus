...
float __h=69.4183; //min:0 max:360
float __s =19.605196; //min:0 max:100
float __b =9.512293; //min:0 max:100
float __op =67.393326; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=10; //min:2 max:10
int __num_max=10; //min:10 max:30

float __pos=0.31744787; //min:0.1 max:0.4
float __size=51.39927; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 7);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(992,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
