...
float __h=123.2236; //min:0 max:360
float __s =23.500305; //min:0 max:100
...
float __pos=0.3382399; //min:0.1 max:0.4
float __size=48.17682; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 14);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(669,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
