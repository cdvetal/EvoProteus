...
float __h=149.50977; //min:0 max:360
float __s =21.429008; //min:0 max:100
float __b =35.262863; //min:0 max:100
float __op =26.624214; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=4; //min:2 max:10
int __num_max=19; //min:10 max:30

float __pos=0.35530126; //min:0.1 max:0.4
float __size=21.301311; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 5);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(346,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
