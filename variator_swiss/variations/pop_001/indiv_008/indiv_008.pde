...
float __h=129.37976; //min:0 max:360
float __s =45.15356; //min:0 max:100
float __b =35.732746; //min:0 max:100
float __op =93.47186; //min:0 max:100

int __num_circles=5; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=27; //min:10 max:30

float __pos=0.32075194; //min:0.1 max:0.4
float __size=190.87325; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 8);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(23,538);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
