...
float __h=186.61128; //min:0 max:360
float __s =81.690865; //min:0 max:100
float __b =26.06342; //min:0 max:100
float __op =38.18374; //min:0 max:100

int __num_circles=3; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=26; //min:10 max:30

float __pos=0.33919543; //min:0.1 max:0.4
float __size=70.23396; //min:20 max:200

void setup() {
surface.setLocation(23,1030);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 12);//variator
  size(300, 200);
