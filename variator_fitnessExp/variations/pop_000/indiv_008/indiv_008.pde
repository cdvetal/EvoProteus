...
float __h=122.80444; //min:0 max:360
float __s =15.381718; //min:0 max:100
float __b =92.788635; //min:0 max:100
float __op =61.008198; //min:0 max:100

int __num_circles=4; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=25; //min:10 max:30

float __pos=0.34689817; //min:0.1 max:0.4
float __size=50.738243; //min:20 max:200

void setup() {
surface.setLocation(669,538);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 8);//variator
  size(300, 200);
