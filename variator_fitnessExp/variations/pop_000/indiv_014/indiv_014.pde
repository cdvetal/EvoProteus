...
float __h=340.08374; //min:0 max:360
float __s =78.34656; //min:0 max:100
float __b =89.20723; //min:0 max:100
float __op =58.97457; //min:0 max:100

int __num_circles=5; //min:1 max:6
int __num_min=10; //min:2 max:10
int __num_max=23; //min:10 max:30

float __pos=0.31260303; //min:0.1 max:0.4
float __size=32.50152; //min:20 max:200

void setup() {
surface.setLocation(669,1030);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 14);//variator
  size(300, 200);
