...
float __h=132.7244; //min:0 max:360
float __s =48.061157; //min:0 max:100
float __b =44.614964; //min:0 max:100
float __op =75.02595; //min:0 max:100

int __num_circles=1; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=15; //min:10 max:30

float __pos=0.36386666; //min:0.1 max:0.4
float __size=154.71597; //min:20 max:200

void setup() {
surface.setLocation(346,538);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 7);//variator
  size(300, 200);
