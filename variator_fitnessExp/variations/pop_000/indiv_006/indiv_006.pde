...
float __h=57.15105; //min:0 max:360
float __s =37.644875; //min:0 max:100
float __b =1.5356302; //min:0 max:100
float __op =55.769176; //min:0 max:100

int __num_circles=3; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=19; //min:10 max:30

float __pos=0.12080132; //min:0.1 max:0.4
float __size=141.48398; //min:20 max:200

void setup() {
surface.setLocation(23,538);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 6);//variator
  size(300, 200);
