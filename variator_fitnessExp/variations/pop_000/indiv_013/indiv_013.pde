...
float __h=187.67249; //min:0 max:360
float __s =89.820274; //min:0 max:100
float __b =11.636156; //min:0 max:100
float __op =62.320538; //min:0 max:100

int __num_circles=3; //min:1 max:6
int __num_min=9; //min:2 max:10
int __num_max=11; //min:10 max:30

float __pos=0.34547737; //min:0.1 max:0.4
float __size=134.57755; //min:20 max:200

void setup() {
surface.setLocation(346,1030);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 13);//variator
  size(300, 200);
