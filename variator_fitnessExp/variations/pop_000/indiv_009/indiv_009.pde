...
float __h=284.32413; //min:0 max:360
float __s =43.355812; //min:0 max:100
float __b =95.6063; //min:0 max:100
float __op =90.706245; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=2; //min:2 max:10
int __num_max=14; //min:10 max:30

float __pos=0.26348945; //min:0.1 max:0.4
float __size=104.05306; //min:20 max:200

void setup() {
surface.setLocation(23,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 9);//variator
  size(300, 200);
