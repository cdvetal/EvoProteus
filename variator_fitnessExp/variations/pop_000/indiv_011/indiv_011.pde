...
float __h=274.99603; //min:0 max:360
float __s =46.46207; //min:0 max:100
float __b =2.2397935; //min:0 max:100
float __op =17.981035; //min:0 max:100

int __num_circles=1; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=19; //min:10 max:30

float __pos=0.2546006; //min:0.1 max:0.4
float __size=106.26744; //min:20 max:200

void setup() {
surface.setLocation(669,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");clientSketches = new Client(this, "localhost", 3000 + 11);//variator
  size(300, 200);
