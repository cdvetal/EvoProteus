...
String input;
int exitValue;//Injected line

boolean __bg=true; //min:0 max:1
float __h=206.68086; //min:0 max:360
float __s =81.201; //min:0 max:100
float __b =50.751293; //min:0 max:100
float __op =68.47607; //min:0 max:100

int __num_circles=6; //min:1 max:6
int __num_min=4; //min:2 max:10
int __num_max=24; //min:10 max:30

float __pos=0.12849483; //min:0.1 max:0.4
float __size=80.97896; //min:20 max:200
...
void setup() {
  ...
  clientSketches = new Client(this, "localhost", 3000 + 0); //variator
  final String sketch = getClass().getName();
  pid = int(ProcessHandle.current().pid());
  pidT = str(pid);
  clientSketches.write(sketch + " " + pidT);
...
void draw() {
  ...
  //clientSketches.write(pidT);
  //clientSketches.write(sketch + " " + listener + " " + p.x + " " + p.y);

  if (clientSketches.available() > 0) {
    input = clientSketches.readString();
    exitValue = int(input);
    if (exitValue == 2) exit();
  }//variator
}
