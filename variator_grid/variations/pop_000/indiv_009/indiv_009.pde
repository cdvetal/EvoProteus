...
float __h=346.86685; //min:0 max:360
float __s =49.485725; //min:0 max:100
float __b =47.654205; //min:0 max:100
float __op =26.058811; //min:0 max:100

int __num_circles=1; //min:1 max:6
int __num_min=5; //min:2 max:10
int __num_max=23; //min:10 max:30

float __pos=0.119702935; //min:0.1 max:0.4
float __size=168.18294; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 9);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(1315,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
