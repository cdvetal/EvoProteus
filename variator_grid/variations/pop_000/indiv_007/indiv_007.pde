...
float __h=354.76288; //min:0 max:360
float __s =32.55751; //min:0 max:100
float __b =58.70859; //min:0 max:100
float __op =84.09166; //min:0 max:100

int __num_circles=4; //min:1 max:6
int __num_min=5; //min:2 max:10
int __num_max=22; //min:10 max:30

float __pos=0.33485535; //min:0.1 max:0.4
float __size=147.98203; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 7);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(669,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
