...
float __h=287.6104; //min:0 max:360
float __s =54.15962; //min:0 max:100
float __b =80.61189; //min:0 max:100
float __op =37.01566; //min:0 max:100

int __num_circles=1; //min:1 max:6
int __num_min=4; //min:2 max:10
int __num_max=14; //min:10 max:30

float __pos=0.2203281; //min:0.1 max:0.4
float __size=171.84; //min:20 max:200

void setup() {
clientSketches = new Client(this, "localhost", 3000 + 8);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(992,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");//Injected line
  size(300, 200);
