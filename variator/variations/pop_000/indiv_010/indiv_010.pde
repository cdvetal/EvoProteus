...
float __h=39.953304; //min:0 max:360
float __s =17.435867; //min:0 max:100
float __b =82.435005; //min:0 max:100
float __op =41.476788; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=9; //min:2 max:10
int __num_max=27; //min:10 max:30

float __pos=0.18884593; //min:0.1 max:0.4
float __size=20.958794; //min:20 max:200

void setup() {
surface.setLocation(346,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 10);//variator
  size(300, 200);
