...
float __h=195.31696; //min:0 max:360
float __s =94.944695; //min:0 max:100
float __b =34.17651; //min:0 max:100
float __op =10.013342; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=28; //min:10 max:30

float __pos=0.32549053; //min:0.1 max:0.4
float __size=34.936398; //min:20 max:200

void setup() {
surface.setLocation(346,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 10);//variator
  size(300, 200);
