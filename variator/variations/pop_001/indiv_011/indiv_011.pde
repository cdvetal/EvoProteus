...
float __h=69.66716; //min:0 max:360
float __s =52.497707; //min:0 max:100
float __b =92.222725; //min:0 max:100
float __op =83.94134; //min:0 max:100

int __num_circles=2; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=23; //min:10 max:30

float __pos=0.3467518; //min:0.1 max:0.4
float __size=150.48048; //min:20 max:200

void setup() {
surface.setLocation(669,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 11);//variator
  size(300, 200);
