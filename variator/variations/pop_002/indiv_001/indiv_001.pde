...
int __c1 =137; // min:0 max:255
int __c2 =116; // min:0 max:255
int __c3 =189; //min:0 max:255
...
int __diameter =51; // min:10 max:80
float __spring =NaN;
float __gravity =0.040019244; // min:0.01 max:0.05
float __friction =NaN;
Ball[] balls = new Ball[__numBalls];

void setup() {
surface.setLocation(486,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 1);//variator
