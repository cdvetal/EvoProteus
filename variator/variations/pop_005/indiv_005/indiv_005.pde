...
float __h=183.09845; //min:0 max:360
float __s =44.83367; //min:0 max:100
float __b =92.26286; //min:0 max:100
float __op =60.743736; //min:0 max:100

int __num_circles=5; //min:1 max:6
int __num_min=7; //min:2 max:10
int __num_max=17; //min:10 max:30

float __pos=0.10567043; //min:0.1 max:0.4
float __size=50.038284; //min:20 max:200

void setup() {
surface.setLocation(669,292);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 5);//variator
  size(300, 200);
