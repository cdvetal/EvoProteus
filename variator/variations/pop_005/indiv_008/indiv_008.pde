...
float __h=289.99915; //min:0 max:360
float __s =91.645874; //min:0 max:100
float __b =93.47197; //min:0 max:100
float __op =96.902565; //min:0 max:100

int __num_circles=4; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=19; //min:10 max:30

float __pos=0.33324948; //min:0.1 max:0.4
float __size=96.141914; //min:20 max:200

void setup() {
surface.setLocation(669,538);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 8);//variator
  size(300, 200);
