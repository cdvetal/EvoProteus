...
float __h=315.7492; //min:0 max:360
float __s =93.33954; //min:0 max:100
float __b =94.81678; //min:0 max:100
float __op =97.28165; //min:0 max:100

int __num_circles=3; //min:1 max:6
int __num_min=3; //min:2 max:10
int __num_max=19; //min:10 max:30

float __pos=0.10140185; //min:0.1 max:0.4
float __size=50.038284; //min:20 max:200

void setup() {
surface.setLocation(669,784);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");v_m = new Client(this, "localhost", 3000 + 11);//variator
  size(300, 200);
