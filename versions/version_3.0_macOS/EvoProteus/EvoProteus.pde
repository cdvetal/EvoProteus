...
int chooseInterval = 100;  // number of generations which run automatically
int chooseSeconds = 20000; // time interval between generations (necessary to avoid errors on CLIP's side) //23500
...
//  mutationScaling = float(hs[5].getOperatorValue());
  ...
  float xGaussian = pop.getGenerations() / 100.0;
  float A = 0.4;  // Amplitude
  float mu = 0;    // Mean
