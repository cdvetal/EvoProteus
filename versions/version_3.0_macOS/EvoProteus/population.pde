...
class Population {
  ...
  void evolve() {
    ...
    for (int i = eliteSize; i < newGeneration.length; ++ i) {
      ...
      //println("Indivudal", i, "Genes:", newGeneration[i].genes);
    }
  ...
  Genotype tournamentSelection() {
    ...
    for (int i = 0; i < tournament.length; ++ i) {
      int random_index;
      if (automatorOn) {
        random_index = int(random(0, individuals.length));
        tournament[i] = individuals[random_index];
      } else {
        random_index = int(random(0, survivors.size()));
        tournament[i] = survivors.get(random_index);
      }
    }
