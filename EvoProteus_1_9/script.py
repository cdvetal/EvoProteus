...
mk = MakeCutouts(cut_size=224, cutn=10)
...
image_features = model.encode_image(into)

input_normed = F.normalize(image_features.unsqueeze(1), dim=2)
embed_normed = F.normalize(text_features.unsqueeze(0), dim=2)

cosine_similarity = torch.cosine_similarity(text_features, image_features, dim=-1).mean()
