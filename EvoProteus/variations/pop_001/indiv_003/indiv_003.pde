float __x =127.37757; //min:0 max:200
float __y =53.980133; //min:0 max:200
float __width =106.75147; //min:70 max:200
float __height =185.17456; //min:70 max:200

float __r =169.52753; //min:1 max:255
float __g =239.72777; //min:1 max:255
float __b =219.05516; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 8);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(992,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
