...
float __y =175.06583; //min:0 max:200
float __width =130.27606; //min:70 max:200
float __height =140.71542; //min:70 max:200

float __r =119.452484; //min:1 max:255
float __g =90.58176; //min:1 max:255
float __b =62.784588; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 41);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(346,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
