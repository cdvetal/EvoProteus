float __x =69.88159; //min:0 max:200
float __y =153.90384; //min:0 max:200
float __width =190.76334; //min:70 max:200
float __height =174.25157; //min:70 max:200

float __r =44.718884; //min:1 max:255
float __g =232.20877; //min:1 max:255
float __b =191.19426; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 2);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(23,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
