float __x =138.16814; //min:0 max:200
float __y =198.63586; //min:0 max:200
float __width =189.3371; //min:70 max:200
float __height =199.8176; //min:70 max:200

float __r =81.90885; //min:1 max:255
float __g =90.03042; //min:1 max:255
float __b =180.17615; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 5);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(346,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
