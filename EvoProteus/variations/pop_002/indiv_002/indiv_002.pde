...
float __y =190.04915; //min:0 max:200
float __width =112.57756; //min:70 max:200
float __height =148.02226; //min:70 max:200

float __r =130.78064; //min:1 max:255
float __g =196.84218; //min:1 max:255
float __b =149.70996; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 12);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(669,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
