...
float __y =182.79475; //min:0 max:200
float __width =146.82924; //min:70 max:200
float __height =171.07834; //min:70 max:200

float __r =169.08707; //min:1 max:255
float __g =125.75762; //min:1 max:255
float __b =32.204845; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 66);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(346,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
