float __x =79.95974; //min:0 max:200
float __y =185.37997; //min:0 max:200
float __width =165.47206; //min:70 max:200
float __height =142.1799; //min:70 max:200

float __r =193.76054; //min:1 max:255
float __g =86.77281; //min:1 max:255
float __b =30.965128; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 65);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(23,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
