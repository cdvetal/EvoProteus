float __x =99.49879; //min:0 max:200
float __y =144.45682; //min:0 max:200
float __width =131.8953; //min:70 max:200
float __height =171.07834; //min:70 max:200

float __r =247.09166; //min:1 max:255
float __g =169.3655; //min:1 max:255
float __b =29.763855; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 68);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(992,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
