float __x =84.53992; //min:0 max:200
float __y =144.45682; //min:0 max:200
float __width =126.073586; //min:70 max:200
float __height =171.07834; //min:70 max:200

float __r =187.22446; //min:1 max:255
float __g =104.95584; //min:1 max:255
float __b =32.204845; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 67);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(669,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
