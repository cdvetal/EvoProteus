float __x =63.891834; //min:0 max:200
float __y =171.90326; //min:0 max:200
float __width =102.09478; //min:70 max:200
float __height =129.58578; //min:70 max:200

float __r =132.64748; //min:1 max:255
float __g =61.54027; //min:1 max:255
float __b =115.61803; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 24);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(1315,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
