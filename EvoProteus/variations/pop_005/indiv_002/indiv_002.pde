float __x =97.18341; //min:0 max:200
float __y =164.64487; //min:0 max:200
float __width =122.8842; //min:70 max:200
float __height =152.74713; //min:70 max:200

float __r =113.628876; //min:1 max:255
float __g =89.67655; //min:1 max:255
float __b =136.35893; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 27);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(669,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
