float __x =185.73524; //min:0 max:200
float __y =88.13459; //min:0 max:200
float __width =93.38524; //min:70 max:200
float __height =103.98456; //min:70 max:200

float __r =89.23542; //min:1 max:255
float __g =15.889762; //min:1 max:255
float __b =171.39935; //min:1 max:255


void setup() {
/*Injected line, you may delete it*/ clientSketches = new Client(this, "localhost", 3000 + 3);pid = int(ProcessHandle.current().pid());pidT = str(pid);clientSketches.write("0" + " " + sketch + " " + pidT);surface.setLocation(23,46);PSurfaceAWT awtSurface = (PSurfaceAWT)surface;smoothCanvas = (PSurfaceAWT.SmoothCanvas)awtSurface.getNative();println("[Client] Client connected");
  size(300, 300);
  background(255);
  pixelDensity(2);
