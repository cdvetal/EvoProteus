...
from metrics import ClipPrompt, Aesthetic2Loss
...
aesthetic = Aesthetic2Loss(model_14, preprocess_14)
...
def image_metric_clip():
    ...
    # print("Prompt", prompt, tensorImg.shape)
...
@app.route('/images/clip_aesthetic', methods=['POST'])
def image_metric_clip_aesthetic():
    tensorImg = load_image(request.data)
    ...
    with torch.no_grad():
        aesthetic_fitness = aesthetic.evaluate(tensorImg)
        aesthetic_fitness = aesthetic_fitness.detach().cpu().numpy().squeeze()

        clip_fitness = clip_prompt.evaluate(tensorImg, prompt)
        clip_fitness = clip_fitness.cpu().numpy().squeeze()

        response_message = {"clip": float(clip_fitness), "aesthetic": float(aesthetic_fitness)}
        # print(response_message)
