...
class Main {
  ...
  void runSketch(int counter) {
    ...
    for (int f = 0; f < counter; ++ f) {
      ...
      if (counter == -1) {
        ...
        Process v = exec("C:\\Users\\cdv\\Documents\\processing-4.3\\processing-java.exe", "--sketch=" + path, "--run");
        process.add(v);
      }
    }
  ...
  void setGrid() {
    ...
    gridX = (sketchW * counterGridX) + (16 * (counterGridX + 1));
    //gridX = (sketchW * counterGridX) + (23 * (counterGridX + 1));
    gridY = (sketchH * counterGridY) + (40 * (counterGridY + 1)); //46
